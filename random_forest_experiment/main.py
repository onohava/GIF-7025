...
from datasets_creation import create_dataset, create_filtered_dataset, filter_dataset, filter_dataset2, split_X_y
import pandas as pd
import joblib
...
def main_process():
    ...
    # Features (everything except magnitude)
    ...
    # Split train data into 10 roughly equal subsets
    subset_size = len(X_train) // 10

    for i in range(10):
        # Determine indices for current subset
        start = 0
        end = (i + 1) * subset_size if i < 9 else len(X_train)  # last subset takes remaining rows

        X_subset = X_train.iloc[start:end]
        y_subset = y_train.iloc[start:end]

        # Increment trees for warm_start
        rf.n_estimators = 50 * (i + 1)  # progressively increase number of trees

        # Train on current cumulative subset
    ...
    # --- Finally, evaluate on eval set ---
    y_pred_eval = rf.predict(X_eval)
    mse_eval = mean_squared_error(y_eval, y_pred_eval)
    print(f"Final Eval MSE: {mse_eval:.4f}")
    joblib.dump(rf, "final_random_forest.pkl")
    print("Model saved to final_random_forest.pkl")
